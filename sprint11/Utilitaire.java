...
public class Utilitaire {
    ...
    public boolean checkMethod(Method method,String sessionName,HttpServletRequest request)throws Exception{ //jerena hoe annoter Auth ve io methode io, de jerena hoe io olona connecte io ve mitovy profil @ le annotation
            Annotation annotation= method.getAnnotation(Auth.class);
            if(annotation==null){ return true; }
            else{
                  AccessAllClassByPackage access=new AccessAllClassByPackage();
                  string valeur= (String)access.getValueAnnotation(method, Auth.class, "profil");
                  boolean exist=false;
                  HttpSession session= request.getSession();
                  Object valSesion=session.getAttribute(valeur);
                  if(valSesion==null){ return false; }
                  else{ return true; }
            }
    }
